...
import dev.userteemu.f5transitions.compatibility.PerspectiveModTransitionHelper;
import dev.userteemu.f5transitions.config.EaseProperty;
import dev.userteemu.f5transitions.config.TransitionsConfig;
import dev.userteemu.f5transitions.perspectives.*;
...
public class TransitionHelper {
    ...
    public double getCameraDistance(float partialTicks) {
        ...
        if (from == null || !transitionActive) return toCameraDistance;
            ...
            return fromCameraDistance + (EaseProperty.DISTANCE.getValue(getProgressOfMaxWithPartialTicks(partialTicks)) * (toCameraDistance - fromCameraDistance));
        }
    }
    ...
    public float getYRotationBonus(float partialTicks) {
        if (from == null || !transitionActive) return this.to.getCameraYRotation();
            ...
            return fromYRotation + (EaseProperty.ROTATION.getValue(getProgressOfMaxWithPartialTicks(partialTicks)) * (toYRotation - fromYRotation));
        }
    }
    ...
    public float getPlayerOpacity() {
        ...
        else return this.from.getPlayerOpacity() + EaseProperty.OPACITY.getValue(progressOfMax) * (this.to.getPlayerOpacity() - this.from.getPlayerOpacity());
    }
